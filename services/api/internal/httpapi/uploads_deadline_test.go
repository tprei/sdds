...
func TestNonUploadRequestRetainsServerReadTimeout(t *testing.T) {
	...
	response, err := sendThrottledRequest(address, http.MethodPost, "/", "application/octet-stream", []byte("body"), 1200*time.Millisecond)
	if err == nil {
		defer func() { _ = response.Body.Close() }()
		if response.StatusCode == http.StatusNoContent {
			t.Fatalf("status = %d, want server read timeout", response.StatusCode)
		}
	}
}
