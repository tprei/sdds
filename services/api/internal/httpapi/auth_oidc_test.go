...
func TestCreateAuthOidcSessionInvalidUsernameDoesNotConsumeLimit(t *testing.T) {
	limits := oidcTestLimits()
	limits.OIDCRequestsPerMinute = 1
	router := newOIDCAuthTestRouter(t, fakeOIDCVerifier{verify: func(_ context.Context, provider oidc.Provider, _ string, _ string) (oidc.Identity, error) {
		return oidc.Identity{Provider: provider, Subject: "subject-1"}, nil
	...
	invalid := postOIDCSession(router, `{"provider":"google","id_token":"token","nonce":"nonce","username":"Has Space"}`)
	requireOpenAPIResponse(t, jsonRequest(http.MethodPost, "/v1/auth/oidc/sessions", ""), invalid)
	requireOIDCError(t, invalid, http.StatusBadRequest, openapi.ErrorCodeInvalidAuth)

	valid := postOIDCSession(router, `{"provider":"google","id_token":"token","nonce":"nonce"}`)
	requireOpenAPIResponse(t, jsonRequest(http.MethodPost, "/v1/auth/oidc/sessions", ""), valid)
	requireOIDCError(t, valid, http.StatusConflict, openapi.ErrorCodeUsernameRequired)
}
