...
func TestDeleteLoginIdentityStripsPasswordCredentialKeepingUsername(t *testing.T) {
	ctx := context.Background()
	now := oidcTestNow()
	db := openMigratedDatabase(t, ctx)
	store := newUserStore(db, func() time.Time { return now })
	passwordUser := createTestPasswordUser(t, ctx, store, now, "alice", "password-strip")
	insertVerifiedEmail(t, ctx, db, passwordUser.User.ID, "alice@example.com", "token")
	if _, err := store.ResolveOIDCIdentity(ctx, user.ResolveOIDCIdentityInput{
		Provider:      user.LoginIdentityProviderGoogle,
		Subject:       "google-strip",
		Email:         "alice@example.com",
		EmailVerified: true,
		TokenHash:     user.HashSessionToken("google-strip"),
		ExpiresAt:     now.Add(user.SessionLifetime),
	}); err != nil {
		t.Fatalf("link strip fixture: %v", err)
	}

	identities, err := store.ListLoginIdentities(ctx, passwordUser.User.ID)
	if err != nil {
		t.Fatalf("list strip identities: %v", err)
	}
	var localID user.LoginIdentityID
	for _, identity := range identities {
		if identity.Provider == user.LoginIdentityProviderLocal {
			localID = identity.ID
		}
	}
	if localID == "" {
		t.Fatal("local identity not found")
	}
	if err := store.DeleteLoginIdentity(ctx, passwordUser.User.ID, localID); err != nil {
		t.Fatalf("disconnect password identity: %v", err)
	}

	var kind string
	var secretHash sql.NullString
	var identifier string
	if err := db.QueryRowContext(ctx, `SELECT kind, secret_hash, normalized_identifier FROM user_login_identities WHERE id = ?`, localID).Scan(&kind, &secretHash, &identifier); err != nil {
		t.Fatalf("read stripped identity: %v", err)
	}
	if kind != string(user.LoginIdentityKindOIDC) || secretHash.Valid || identifier != "alice" {
		t.Fatalf("stripped identity = (%s, valid=%t, %s), want the username-only row (oidc, no secret, alice)", kind, secretHash.Valid, identifier)
	}
	remaining, err := store.ListLoginIdentities(ctx, passwordUser.User.ID)
	if err != nil {
		t.Fatalf("list remaining strip identities: %v", err)
	}
	if len(remaining) != 1 || remaining[0].Provider != user.LoginIdentityProviderGoogle {
		t.Fatalf("remaining identities = %+v, want only google", remaining)
	}
	current, err := store.FindCurrentSession(ctx, user.HashSessionToken("google-strip"), now)
	if err != nil {
		t.Fatalf("find google session after strip: %v", err)
	}
	if current.Username != "alice" {
		t.Fatalf("username after strip = %q, want alice", current.Username)
	}
	if _, err := store.FindPasswordLogin(ctx, "alice"); !errors.Is(err, user.ErrInvalidCredentials) {
		t.Fatalf("password login after strip error = %v, want ErrInvalidCredentials", err)
	}

	if _, err := db.ExecContext(ctx,
		`INSERT INTO user_contact_channel_tokens (id, channel_id, purpose, token_hash, created_at, expires_at, consumed_at) VALUES ('reset-strip', ?, 'reset', ?, ?, ?, NULL)`,
		"verified-"+string(passwordUser.User.ID), user.HashContactChannelToken("reset-strip-token"), unixMillis(now), unixMillis(now.Add(time.Hour)),
	); err != nil {
		t.Fatalf("seed reset token: %v", err)
	}
	channels := NewContactChannelStore(db)
	if _, err := channels.ConsumeTokenAndSetPassword(ctx, user.HashContactChannelToken("reset-strip-token"), "restored-hash", now); err != nil {
		t.Fatalf("reset password after strip: %v", err)
	}
	login, err := store.FindPasswordLogin(ctx, "alice")
	if err != nil {
		t.Fatalf("password login after reset: %v", err)
	}
	if login.SecretHash != "restored-hash" {
		t.Fatalf("secret hash after reset = %q, want restored-hash", login.SecretHash)
	}
}
